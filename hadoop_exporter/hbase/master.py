...
class HBaseMasterMetricCollector(MetricCollector):
    ...
    def __init__(self, cluster, url):
        ...
        self._hbase_master_metrics = {}
        for i in range(len(self._file_list)):
            self._hbase_master_metrics.setdefault(self._file_list[i], {})
    ...
    def collect(self):
        ...
        try:
            ...
            self._hbase_master_metrics.update(common_metrics())

            for i in range(len(self._merge_list)):
                service = self._merge_list[i]
                for metric in self._hbase_master_metrics[service]:
                    yield self._hbase_master_metrics[service][metric]
    ...
    def _setup_server_labels(self):
        for metric in self._metrics['Server']:
            ...
            if 'RegionServersState' in metric:
                ...
                pass
            self._hbase_master_metrics['Server'][metric] = GaugeMetricFamily("_".join([self._prefix, 'server', name]),
    ...
    def _setup_balancer_labels(self):
        ...
        for metric in self._metrics['Balancer']:
            ...
            if '_min' in metric or '_max' in metric or '_mean' in metric or 'median' in metric:
                name = re.sub('([a-z0-9])([A-Z])', r'\1_\2', metric).lower()
                self._hbase_master_metrics['Balancer'][metric] = GaugeMetricFamily("_".join([self._prefix, name]),
                ...
                if balancer_flag:
                    ...
                    key = 'BalancerCluster'
                    self._hbase_master_metrics['Balancer'][key] = HistogramMetricFamily("_".join([self._prefix, name]),
    ...
    def _setup_assignmentmanger_labels(self):
        ...
        for metric in self._metrics['AssignmentManger']:
            ...
            if '_min' in metric or '_max' in metric or '_mean' in metric or 'median' in metric:
                name = re.sub('([a-z0-9])([A-Z])', r'\1_\2', metric).lower()
                self._hbase_master_metrics['AssignmentManger'][metric] = GaugeMetricFamily("_".join([self._prefix, name]),
                ...
                if bulkassign_flag:
                    ...
                    key = 'BulkAssign'
                    self._hbase_master_metrics['AssignmentManger'][key] = HistogramMetricFamily("_".join([self._prefix, name]),
    ...
    def _setup_ipc_labels(self):
        ...
        for metric in self._metrics['IPC']:
            ...
            if '_min' in metric or '_max' in metric or '_mean' in metric or 'median' in metric:
                ...
                self._hbase_master_metrics['IPC'][metric] = GaugeMetricFamily("_".join([self._prefix, name]),
                                                                              self._metrics['IPC'][metric],
                                                                              labels=label)
            elif 'RangeCount_' in metric:
                name = metric.replace("-", "_").lower()
                self._hbase_master_metrics['IPC'][metric] = GaugeMetricFamily("_".join([self._prefix, 'ipc', name]),
                                                                              self._metrics['IPC'][metric],
                                                                              labels=label)
                ...
                if total_calltime_flag:
                    ...
                    key = 'TotalCallTime'
                    self._hbase_master_metrics['IPC'][key] = HistogramMetricFamily("_".join([self._prefix, name]),
                ...
                if exception_flag:
                    ...
                    key = 'exceptions'
                    label.append("type")
                    self._hbase_master_metrics['IPC'][key] = GaugeMetricFamily("_".join([self._prefix, name]),
    ...
    def _setup_filesystem_labels(self):
        ...
        for metric in self._metrics['FileSystem']:
            ...
            if '_min' in metric or '_max' in metric or '_mean' in metric or 'median' in metric:
                name = snake_case
                self._hbase_master_metrics['FileSystem'][metric] = GaugeMetricFamily("_".join([self._prefix, name]),
                ...
                if metahlog_split_time_flag:
                    ...
                    self._hbase_master_metrics['FileSystem'][key] = HistogramMetricFamily("_".join([self._prefix, name]),
        ...
        for metric in self._metrics['Server']:
            if 'RegionServersState' in metric:
                if 'tag.liveRegionServers' in bean and bean['tag.liveRegionServers']:
                    live_region_servers = yaml.safe_load(
                        bean['tag.liveRegionServers'])
                    ...
                    for j in range(len(live_region_list)):
                        ...
                        self._hbase_master_metrics['Server'][metric].add_metric(
                            label, 1.0)
                elif 'tag.deadRegionServers' in bean and bean['tag.deadRegionServers']:
                    dead_region_servers = yaml.safe_load(
                        bean['tag.deadRegionServers'])
    ...
    def _get_balancer_metrics(self, bean):
        ...
        for metric in self._metrics['Balancer']:
            if '_min' in metric or '_max' in metric or '_mean' in metric or 'median' in metric:
                self._hbase_master_metrics['Balancer'][metric].add_metric(
                    label, bean[metric] if metric in bean and bean[metric] else 0)
            elif 'BalancerCluster' in metric:
        ...
        self._hbase_master_metrics['Balancer'][key].add_metric(
            label, buckets=balancer_bucket, sum_value=balancer_sum)
    ...
    def _get_assignmentmanger_metrics(self, bean):
        ...
        for metric in self._metrics['AssignmentManger']:
            if '_min' in metric or '_max' in metric or '_mean' in metric or 'median' in metric:
                self._hbase_master_metrics['AssignmentManger'][metric].add_metric(
                    label, bean[metric] if metric in bean and bean[metric] else 0)
            elif 'BulkAssign' in metric:
                if '_num_ops' in metric:
        ...
        self._hbase_master_metrics['AssignmentManger'][bulkassign_key].add_metric(
            label, buckets=bulkassign_bucket, sum_value=bulkassign_sum)
        self._hbase_master_metrics['AssignmentManger'][assign_key].add_metric(
            label, buckets=assign_bucket, sum_value=assign_sum)
    ...
    def _get_ipc_metrics(self, bean):
        ...
        for metric in self._metrics['IPC']:
            if '_min' in metric or '_max' in metric or '_mean' in metric or 'median' in metric or 'RangeCount_' in metric:
                self._hbase_master_metrics['IPC'][metric].add_metric(
                    label, bean[metric] if metric in bean and bean[metric] else 0)
            elif 'TotalCallTime' in metric:
                if '_num_ops' in metric:
                ...
                if 'exceptions' == metric:
                    ...
                    type = metric.split(".")[1]
                self._hbase_master_metrics['IPC'][key].add_metric(new_label + [type],
                                                                  bean[metric] if metric in bean and bean[metric] else 0)
            else:
                self._hbase_master_metrics['IPC'][metric].add_metric(
                    label, bean[metric] if metric in bean and bean[metric] else 0)

        total_calltime_bucket = zip(
            total_calltime_percentile, total_calltime_value)
        total_calltime_bucket.sort()
        total_calltime_bucket.append(("+Inf", total_calltime_count))
        process_calltime_bucket = zip(
            process_calltime_percentile, process_calltime_value)
        process_calltime_bucket.sort()
        process_calltime_bucket.append(("+Inf", process_calltime_count))
        queue_calltime_bucket = zip(
            queue_calltime_percentile, queue_calltime_value)
        ...
        self._hbase_master_metrics['IPC'][total_calltime_key].add_metric(
            label, buckets=total_calltime_bucket, sum_value=total_calltime_sum)
        self._hbase_master_metrics['IPC'][process_calltime_key].add_metric(
            label, buckets=process_calltime_bucket, sum_value=process_calltime_sum)
        self._hbase_master_metrics['IPC'][queue_calltime_key].add_metric(
            label, buckets=queue_calltime_bucket, sum_value=queue_calltime_sum)
        self._hbase_master_metrics['IPC'][response_key].add_metric(
            label, buckets=response_bucket, sum_value=response_sum)
        self._hbase_master_metrics['IPC'][request_key].add_metric(
            label, buckets=request_bucket, sum_value=request_sum)
    ...
    def _get_filesystem_metrics(self, bean):
        ...
        for metric in self._metrics['FileSystem']:
            if '_min' in metric or '_max' in metric or '_mean' in metric or 'median' in metric or 'RangeCount_' in metric:
                self._hbase_master_metrics['FileSystem'][metric].add_metric(
                    label, bean[metric] if metric in bean and bean[metric] else 0)
            elif 'MetaHlogSplitTime' in metric:
                if '_num_ops' in metric:
                    ...
                    per = metric.split("_")[1].split("th")[0]
                    metahlog_split_time_percentile.append(
                        str(float(per) / 100.0))
                ...
                if '_num_ops' in metric:
                    ...
                    per = metric.split("_")[1].split("th")[0]
                    metahlog_split_size_percentile.append(
                        str(float(per) / 100.0))
        ...
        hlog_split_time_bucket = zip(
            hlog_split_time_percentile, hlog_split_time_value)
        hlog_split_time_bucket.sort()
        hlog_split_time_bucket.append(("+Inf", hlog_split_time_count))
        metahlog_split_time_bucket = zip(
            metahlog_split_time_percentile, metahlog_split_time_value)
        metahlog_split_time_bucket.sort()
        metahlog_split_time_bucket.append(("+Inf", metahlog_split_time_count))
        hlog_split_size_bucket = zip(
            hlog_split_size_percentile, hlog_split_size_value)
        hlog_split_size_bucket.sort()
        hlog_split_size_bucket.append(("+Inf", hlog_split_size_count))
        metahlog_split_size_bucket = zip(
            metahlog_split_size_percentile, metahlog_split_size_value)
        metahlog_split_size_bucket.sort()
        metahlog_split_size_bucket.append(("+Inf", metahlog_split_size_count))

        self._hbase_master_metrics['FileSystem'][hlog_split_time_key].add_metric(
            label, buckets=hlog_split_time_bucket, sum_value=hlog_split_time_sum)
        self._hbase_master_metrics['FileSystem'][metahlog_split_time_key].add_metric(
            label, buckets=metahlog_split_time_bucket, sum_value=metahlog_split_time_sum)
        self._hbase_master_metrics['FileSystem'][hlog_split_size_key].add_metric(
            label, buckets=hlog_split_size_bucket, sum_value=hlog_split_size_sum)
        self._hbase_master_metrics['FileSystem'][metahlog_split_size_key].add_metric(
            label, buckets=metahlog_split_size_bucket, sum_value=metahlog_split_size_sum)
